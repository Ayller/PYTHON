...
while True:
    ...
    escolha = input(
        "Digite o número do livro que deseja emprestar ou '0' para encerrar: "
    )
    if escolha == "0":
        ...
        print(
            f"Livro '{livros_disponiveis[int(escolha)-1]}' emprestado para {nome}. Data de devolução: {data_devolucao.strftime('%Y-%m-%d')}"
        )
        livros_disponiveis.remove(livros_disponiveis[int(escolha) - 1])
    else:
        print(
            "Livro não disponível. Por favor, escolha outro livro ou digite '0' para encerrar."
        )
