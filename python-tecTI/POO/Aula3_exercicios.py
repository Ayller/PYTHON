...
class Mercearia:
    ...
    def comprar(self, quantidade_comprada):
        if quantidade_comprada <= self.quantidade:
            ...
            print(
                f"Você comprou {quantidade_comprada} {self.nome}(s) por R${total:.2f}."
            )
        else:
            print(
                f"Desculpe, não temos {quantidade_comprada} {self.nome}(s) disponíveis. Temos apenas {self.quantidade}."
            )
